...
| `mutations/` | The campaigns themselves: one `mutate_<step>.py` and one `spec_<step>.txt` per step. **`tools/mutations/spec_*.txt` is the authority on the count: 160 must-kill mutations across 19 campaigns** (v23 K1–K5+K4b, v24 M1–M8, v25 N1–N8 minus N7, v26 Q1–Q9, v27 R1–R6, v28 X1–X10, v29 Y1–Y9, v33 S1+S2b, v35 U2+U3, v46 E2–E7, v47 S1–S7 (S8 retired), v48a D1–D6+D8, v48b C1–C7, v49a T1–T5+A1–A2+X1–X2+V1–V2, v49b, v49c W1–W15 (W6 retired), v49d G1–G15, v50a H1–H9, v50b J1–J13). Specs carry each mutation's *final* expectation string, the one that killed in its step's exit report.<br><br>**The nightly runs fifteen campaigns on Linux against a fresh ASan tree, and v35 on macOS**, because its mutations anchor inside `bench_common.c`'s `#if defined(__APPLE__)` branch and cannot even be applied on Linux — which is the point, since U2 is a defect that only ever appeared on a macOS VM. `spec_v35.txt` lists only U2/U3: U1 mutates an **x86_64-only** branch that does not compile on arm64, so it is applied by hand during `bench.yml`'s bring-up, where the agreement check kills it. **v49d, v50a and v50b are not in it yet, and neither is `check_mutation_anchors.py`.** The edit exists — `.github/workflows/nightly.yml` on the `nightly-bringup` branch adds the three campaigns, the `ws` fuzz target, and an anchors job that GATES the campaigns (`needs:`) rather than running beside them, because a rotted anchor aborts the runner and learning that after three hours of mutation runs costs three hours to discover what five minutes knew. It is not on `main` because a workflow reaches `main` only after it has gone green on its own bring-up branch (V3-3 decision 1), and pushing that branch is an outward-facing action this project takes only on instruction. Until then the nightly verifies **123 of the 160** mutations it claims authority over, and this sentence is the honest number.<br><br>**A step's campaign is committed with the step**, and a deferral gets a stated expiry: until V3-4 these files lived only in one agent's scratch directory, which made every "re-run" claim unreproducible by anyone else, and v48b, v49a–c, v49d and v50a each waited one step for a bring-up that was named when the deferral was made. V4-10b discharged the last of them.<br><br>Four mutations are deliberately absent as documented EQUIVALENT MUTANTS, because a documented survivor cannot be a pass criterion. **v48b's C8** (the decoy/no-user-id guard before a login code is issued) survives because a decoy pin has no secret key, so no sig_A over it can ever verify and the guarded state is unreachable through the daemon's own interface; the guard stays in the code. **v48a's D7** (`conn_io_push` no longer honours the sticky `failed` flag) survives because the offending frame header is still at offset 0 and is re-read and re-refused by `note_header()` either way. **N7** (keys committed before `consume_success`) survived in V2-5 and V2-10: `fail_ctx()` wipes the keys on every post-commit failure path, so no public-API test can distinguish the reordering. **v50b's absent PROXY guard** (widening `note_header`'s condition to name the PROXY stage) survives because the preamble is consumed strictly inside `WS_STAGE_UPGRADE`, which the existing condition already excludes — the two versions are indistinguishable, and the argument is written next to the guard. |
