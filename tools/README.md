...
| `check_build_current.sh` | Proves a build directory's binaries were compiled from the **current** working tree — it rebuilds and requires every project object and executable to be byte-identical afterwards — before any suite result from it is reported. Since V4-13b it also reads an Emscripten tree: `.wasm` modules are its executables and are fingerprinted whole (no text section to dump, no build-id to move); a changed `client_core.c` left unbuilt is reported STALE. **It rebuilds from scratch** (every project object, archive and executable deleted first, never `_deps`): macOS's make 3.81 compares timestamps to the second, so a source restored within the second of the last link was invisible to the old no-op rebuild (F87). |
