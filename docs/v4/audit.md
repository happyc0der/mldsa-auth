...
| F19 | **`-Werror=format-truncation` under gcc at `-O2 -D_FORTIFY_SOURCE=2`**: `snprintf(out, cap, "%s/%s", g_dir, name)` in a test helper. gcc cannot prove the result fits and promotes it to an error; clang does not diagnose it, and CI builds gcc at `-O0`, so **no existing gate sees it**. Found by the V4-8b pre-push container check at `-O2`. Same class as F0: a latent blocker for a **Release+gcc** build, which packaging needs. | v4 (V4-6) | Low | `tests/test_authd_keyfile.c:34` | widen the buffer or bound the inputs; not fixed in V4-8b to keep that step's scope honest | V4-11 |
