...
/* Exit statuses, the operator's KDF parameters and the shared helpers
 * (join, path_exists, unexpected, need, failed, hexify, read_pass) are in
 * cli_common.h since V4-13a, shared with client_cli.c. */

/* Argon2id for the SERVER's identity, spec 12. The operator's value is in
 * cli_common.h; this one has no user outside this file. */
...
/* authd_client -- the device's tool -- is client_cli.c since V4-13a, built on
 * client_core.c. */
