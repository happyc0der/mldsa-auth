 ...
 *
 * V4-8b implements LOGIN_CODE, ERROR and BYE. ROTATE/ROTATE_ACK are V4-9's;
 * their ops are named here so the decoder can RECOGNISE them and the
 * connection layer can refuse them with ERROR(NOT_PERMITTED) rather than
 * with the malformed status, which would be a less honest answer.
 */
